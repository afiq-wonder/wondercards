...
interface WonderCelebrateScreenProps {
  card: WonderCard;
  wonderScore: number;
  savedMoment: string;
  completed: boolean;
...
export default function WonderCelebrateScreen({
  card,
  wonderScore,
  savedMoment,
  completed,
  onPrimary,
}: WonderCelebrateScreenProps) {
  return (
    <article className="overflow-hidden rounded-[2rem] border border-white/70 bg-white/78 shadow-[0_20px_60px_rgba(15,23,42,0.10)] backdrop-blur">
      <div className="relative">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(191,219,254,0.12),transparent_35%),radial-gradient(circle_at_top_right,rgba(196,181,253,0.10),transparent_30%),linear-gradient(180deg,rgba(255,255,255,0.95),rgba(255,255,255,0.76))]" />
        ...
        <div className="relative p-8 md:p-10">
          ...
          <div className="relative mt-8 grid gap-4 md:grid-cols-2">
            <div className="rounded-[1.6rem] bg-sky-50 p-6 ring-1 ring-sky-100">
              ...
              <p className="mt-3 leading-7 text-slate-700">
                Emotional progress through this adventure.
              </p>
            </div>
            ...
            <div className="rounded-[1.6rem] bg-white p-6 ring-1 ring-slate-200/70">
              ...
              {savedMoment.trim().length > 0 ? (
                <p className="mt-3 leading-7 text-slate-700">
                  {savedMoment}
                </p>
              ) : (
                <p className="mt-3 leading-7 text-slate-500">
                  No Wonder Moment was saved yet.
                </p>
              )}
            </div>
          </div>

          <div className="relative mt-8 rounded-[1.6rem] bg-cyan-50 p-6 ring-1 ring-cyan-100">
          ...
          <button
            type="button"
            onClick={onPrimary}
            className="relative mt-8 w-full rounded-2xl bg-gradient-to-r from-sky-300 via-cyan-200 to-violet-200 px-6 py-4 text-base font-bold text-slate-800 shadow-sm shadow-cyan-100/60 transition hover:brightness-105 active:scale-[0.99]"
          >
            {completed
              ? "Play Again"
              : "Finish Adventure"}
